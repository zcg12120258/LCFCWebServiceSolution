    ...
    public class DataProcessHelper
    {
        ...
        public ReturnResult Get_PPM_LotFromSap_Process(List<LotMoEntity> lotMoList)//LotEntity
        {
            ...
            if (lotMoList != null && lotMoList.Count > 0)
            {
                ...
                var orderlotMoList = lotMoList.OrderBy(x => x.LOT_NO).ThenByDescending( x => int.Parse(x.MO_NO)).ToList();
                //每页条数   
                int pageSize = 900;
                //页码 0也就是第一条 
                int pageNum = 0;
                while (pageNum * pageSize < orderlotMoList.Count)
                {
                    //分页   
                    var pagelotMoList = orderlotMoList.Skip(pageNum * pageSize).Take(pageSize).ToList();
                    var monumberList = pagelotMoList.Select(x => x.MO_ORDER).ToArray();
                    ...
                    if (toSapList != null && toSapList.Count > 0)
                    {
                        ...
                        var IsEqualList = pagelotMoList.Where(x => !toSapList.Exists(y => x.MO_ORDER.Contains(y.MO_ORDER))).ToList();
                        if (IsEqualList.Count == 0)
                        {
                            var jobStatusTwoList = toSapList.Where(x => x.JOB_STATUS == "2").ToList();
                            if (jobStatusTwoList != null && jobStatusTwoList.Count > 0)
                            {
                                dataProviderService.InsertBatch_R_ISSUE_DESC_T(jobStatusTwoList);
                            }

                            pagelotMoList.ForEach(m =>
                            {
                            ...
                            List<LotMoEntity> lotMoListSort = pagelotMoList.OrderBy(x => int.Parse(x.MO_NO)).GroupBy(y => y.LOT_NO).Select(z => z.FirstOrDefault()).ToList();//根据mo顺序排序，并按lot号分组，得到唯一lot的最小mo序列值
                            ...
                            List<LotMoEntityGroup> LotMoEntityGroupList = pagelotMoList.GroupBy(x => new { x.LINE_NAME, x.SHIFT, x.CREATE_TIME }).Select(g =>
                            ...
                            {
                                x.LotMoEntityList = pagelotMoList.Where(y => x.LINE_NAME == y.LINE_NAME && x.CREATE_TIME == y.CREATE_TIME && x.SHIFT == y.SHIFT).ToList();
                                ...
                                // result = dataProviderService.UpdateBatch_R_WIP_LOT_MO_LOG_T(x.LotMoEntityList);
                            });
            ...
            if (!result.Status)
            {
                dataProviderService.Insert_R_ISSUE_DESC_T(result.Message);
