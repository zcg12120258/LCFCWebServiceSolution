        ...
        public ReturnResult Get_PPM_LotFromSap_Process(List<LotMoEntity> lotMoList)//LotEntity
        {
            ...
            if (toSapList != null && toSapList.Count > 0)
            {
                ...
                if (lotMoList != null && lotMoList.Count > 0)
                {
                    ...
                    //而在toSapList==[job_status=1:传送给Sap的数据列表]中不存的值
                    ...
                    if (IsEqualList.Count > 0)
                    {
                        ...
                        //根据在lotMoList中存在，在R_WIP_LOT_MO_LOG_T表中JobStatus！=1不存在的工单号，查询R_WIP_LOT_MO_LOG_T表中的数据
                        List<LotMoEntity> MoListByMONuber = dataProviderService.GetMoEntityListByMoOrders(string.Join("','", Mo_OrderList));
                        if (MoListByMONuber != null && MoListByMONuber.Count > 0)
                        {
                            var IsEqualList2 = IsEqualList.Where(x => !MoListByMONuber.Exists(y => x.MO_ORDER.Contains(y.MO_ORDER))).ToList();
                            if (IsEqualList2.Count > 0)
                            {
                                //根据在lotMoList中存在，在R_WIP_LOT_MO_LOG_T表中不存在的工单号
                                result.Status = false;
                                result.Message = "工单号:" + string.Join(",", IsEqualList2.Select(x => x.MO_ORDER).ToArray()) + " 在SFISM4.R_WIP_LOT_MO_LOG_T表中不存在";
            ...
            {
                var Mo_OrderList = lotMoList.Select(x => x.MO_ORDER).ToArray();
                //根据在lotMoList中存在，在R_WIP_LOT_MO_LOG_T表中JobStatus！=1不存在的工单号，查询R_WIP_LOT_MO_LOG_T表中的数据
                List<LotMoEntity> MoListByMONuber = dataProviderService.GetMoEntityListByMoOrders(string.Join("','", Mo_OrderList));
                if (MoListByMONuber != null && MoListByMONuber.Count > 0)
                {
                    var IsEqualList2 = lotMoList.Where(x => !MoListByMONuber.Exists(y => x.MO_ORDER.Contains(y.MO_ORDER))).ToList();
                    if (IsEqualList2.Count > 0)
                    {
                        //根据在lotMoList中存在，在R_WIP_LOT_MO_LOG_T表中不存在的工单号
                        result.Status = false;
